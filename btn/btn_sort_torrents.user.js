...
// @version      1.0.0-BEY0NDER
// @description  Sort torrents by SD/HD, then source, then resolution, then origin, then by size
// @icon	     https://broadcasthe.net/favicon.ico
// @updateURL    https://raw.githubusercontent.com/4n0n3000/pt-scripts/main/btn/btn_sort_torrents.user.js
// @downloadURL  https://raw.githubusercontent.com/4n0n3000/pt-scripts/main/btn/btn_sort_torrents.user.js
// @author       Nealefelaen
...
function get_details(tr)
{
    ...
    //var torrent_meta = as[as.length-1].getElementsByTagName('span');
    var torrent_meta = as[as.length-1].textContent.split('/');
    var meta_offset = 0;
    if(torrent_meta[2].trim() == 'NFO')
        meta_offset = 1;
    result.quality = torrent_meta[3+meta_offset].trim();
    result.source = torrent_meta[2+meta_offset].replace(/-/g, '').trim();
    result.origin = torrent_meta[4+meta_offset].trim();
    try
    {
        result.origin = torrent_meta[4+meta_offset].trim();
    }
    ...
    /*result.quality=torrent_meta[6].innerHTML;
    result.source=torrent_meta[0].innerHTML;
    result.origin=torrent_meta[4].innerHTML;*/
