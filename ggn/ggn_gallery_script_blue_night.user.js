// ==UserScript==
// @name         GGn Torrent Gallery - Blue Night
// @namespace    https://github.com/4n0n3000/pt-scripts
// @downloadURL  https://raw.githubusercontent.com/4n0n3000/pt-scripts/main/ggn/ggn_gallery_script_blue_night.user.js
// @updateURL    https://raw.githubusercontent.com/4n0n3000/pt-scripts/main/ggn/ggn_gallery_script_blue_night.user.js
// @version      1.0.0
// @description  GGn Torrent Gallery Script (Blue Night theme variant)
...
// Blue Night palette sourced from https://nowtl.github.io/ggn-blue-night/blue-night_regular.css
...
// Blue Night theme colors (see :root in blue-night_regular.css)
const BLUE_NIGHT_COLORS = {
    SURFACE_0: '#0b0d19',
    SURFACE_2: '#191d2a',
    SURFACE_4: '#232a3d',
    SURFACE_5: '#262b39',
...
const BLUE_NIGHT_ICON_BASE = 'https://nowtl.github.io/ggn-blue-night/assets/icons/ui';
        ...
        GM_config.init({
            ...
            title: `
                <div>
                    ...
                    <div style="margin-top:15px"><small style="font-weight: 300; color: #99a1b3; display: block; font-size: 0.6rem; margin-top: 5px;"><i>Hover over settings marked with * to see more information</i></small></div>
                </div>
            `,
            fields: {
            ...
            css: `
                #GGN_Gallery_Config { background: #191d2a; margin: 0; padding: 10px 20px; color: #c7d0e0; font-family: 'Open Sans', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
                ...
                #GGN_Gallery_Config .config_var { display: flex; flex-direction: row; text-align: left; justify-content: space-between; align-items: center; width: 90%; margin-left: 26px; padding: 4px 0; border-bottom: 1px solid #3a4056; margin-top: 5px; margin-bottom: 5px; }
                #GGN_Gallery_Config .field_label { color: #c7d0e0; width: 45%; user-select: none; font-weight: 500; }
                #GGN_Gallery_Config .field_label.disabled { color: #6b7080; }
                #GGN_Gallery_Config input[type="text"],
                #GGN_Gallery_Config input[type="number"],
                #GGN_Gallery_Config select { background: #262b39; color: #e6ebf5; border: 1px solid #3a4056; border-radius: 3px; box-sizing: border-box; font-size: 0.9em; padding: 8px; width: 55%; transition: all 0.3s ease; }
                #GGN_Gallery_Config input[type="text"]:focus,
                #GGN_Gallery_Config input[type="number"]:focus,
                #GGN_Gallery_Config select:focus { border-color: #4281da; box-shadow: 0 0 5px rgba(66, 129, 218, 0.6); outline: none; }
                #GGN_Gallery_Config input[type="checkbox"] { cursor: pointer; margin-right: 4px !important; width: 16px; height: 16px; accent-color: #4281da; }
                #GGN_Gallery_Config .reset { color: #99a1b3; text-decoration: none; user-select: none; }
                ...
                #GGN_Gallery_Config_resetLink { text-transform: lowercase; background: transparent; color: #99a1b3; }
                ...
                #GGN_Gallery_Config_saveBtn { grid-column: 2; grid-row: 1; background-color: #4281da; color: #FFFFFF; border: none; border-radius: 5px; padding: 15px 20px; font-size: 1rem; font-weight: 500; cursor: pointer; will-change: background-color, transform; transition: background-color 0.2s ease, transform 0.2s ease; box-shadow: 0 2px 5px rgba(0,0,0,0.3); padding-top: 6px !important; padding-bottom: 6px !important; }
                #GGN_Gallery_Config_saveBtn:hover { background-color: #5490e0; transform: translateY(-2px); }
                #GGN_Gallery_Config_saveBtn:active { background-color: #69c077; transform: translateY(1px); }
                #GGN_Gallery_Config_saveBtn.success { box-shadow: 0 0 6px 3px rgba(105, 192, 119, 0.6); }
                #GGN_Gallery_Config_closeBtn { grid-column: 3; grid-row: 1; background-color: #2f3547; color: #c7d0e0; border: 1px solid #3a4056; border-radius: 5px; padding: 15px 20px; font-size: 1rem; font-weight: 500; cursor: pointer; will-change: background-color, transform; transition: background-color 0.2s ease, transform 0.2s ease; box-shadow: 0 2px 5px rgba(0,0,0,0.3); padding-top: 6px !important; padding-bottom: 6px !important; }
                #GGN_Gallery_Config_closeBtn:hover { background-color: #363d52; transform: translateY(-2px); }
                #GGN_Gallery_Config_closeBtn:active { background-color: #232a3d; transform: translateY(1px); }
                /* Tooltip styling */
                #GGN_Gallery_Config .field_label[title]:hover::after { content: attr(title); position: absolute; background: #2f3547; color: #c7d0e0; border: 1px solid #3a4056; padding: 5px 10px; border-radius: 3px; font-size: 0.8em; max-width: 300px; z-index: 100; margin-top: 25px; left: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.3); }
                /* Animations */
                @keyframes pulse { 0%,100%{box-shadow:0 0 6px 2px rgba(66,129,218,0.5);} 50%{box-shadow:0 0 10px 4px #69c077;} }
            `
        });
...
function injectGalleryStyles() {
    ...
    style.innerHTML = `
	  :root{--ggn-card-bg:#232a3d;--ggn-card-bg2:#2f3547;--ggn-card-line:#3a4056;--ggn-text:#c7d0e0;--ggn-muted:#99a1b3;--ggn-overlay:rgba(0,0,0,0.35);}
	  #gallery_view{background:linear-gradient(var(--ggn-card-bg2),var(--ggn-card-bg));padding:8px;border-radius:5px;border:1px solid var(--ggn-card-line)}
	  ...
	  #galleryToggle:hover{text-decoration:underline;color:#fff}
	  #collageBody{position:relative;display:grid;grid-template-columns:repeat(auto-fill,minmax(var(--ggn-width, 220px), 1fr));gap:10px;float:inline-start;margin-top:10px;background:rgba(11,13,25,0.35);padding:8px;border-radius:5px;width:-moz-available;}
	  .groupWrapper{height:auto;box-sizing:border-box;overflow:hidden;background:linear-gradient(var(--ggn-card-bg2),var(--ggn-card-bg));border:1px solid var(--ggn-card-line);border-radius:5px;box-shadow:0 0.5rem 1rem rgba(0,0,0,0.15);transition:scale .5s ease, box-shadow .15s ease;position:relative}
	  ...
	  .releaseTitle a{color:#a0a6b7}
	  .releaseTitle a:hover{color:#fff}
      ...
      .hiddenType{color:#6b7080 !important;text-decoration:line-through}
	`;
...
function maskedIconHtml(iconFile, color) {
    const url = `${BLUE_NIGHT_ICON_BASE}/${iconFile}`;
    return `<span style="display:inline-block;width:12px;height:12px;vertical-align:middle;background-color:${color};-webkit-mask:url('${url}') center / contain no-repeat;mask:url('${url}') center / contain no-repeat;"></span>`;
}
...
function createReleaseFooter(group) {
    ...
    const pinIcon = maskedIconHtml('pin.svg', BLUE_NIGHT_COLORS.ACCENT);
    const checkIcon = group.checked
        ? maskedIconHtml('badge-check.svg', BLUE_NIGHT_COLORS.POSITIVE)
        : maskedIconHtml('x.svg', BLUE_NIGHT_COLORS.NEGATIVE);
    ...
    releaseFooter.innerHTML = `
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 5px;">
            <span style="font-size: var(--footer-font-size); visibility: ${stickyVisibility}">${pinIcon}</span>
            <span style="font-size:calc(var(--footer-font-size) - var(--footer-font-minus))">${group.size}</span>
            <span style="font-size: var(--footer-font-size)">${checkIcon}</span>
        </div>`;
...
function formatRatingDisplay(rating) {
    ...
    if (numericRating < 0) {
        return {
            text: rating,
            style: `color: ${BLUE_NIGHT_COLORS.NEGATIVE};font-weight:600`
        };
    }
    ...
    if (numericRating > 0) {
        return {
            text: rating,
            style: `color: ${BLUE_NIGHT_COLORS.POSITIVE};font-weight:600`
        };
    }
